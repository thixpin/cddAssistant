...
export class DataService {
  ...
  public setVillage(village_code){
    ...
    if(village.length  > 0){
      this.current_village = village[0];
      this.tbl_setting.setItem('village', this.current_village);
    }
  }

  public getCurrentVillage(){
    return this.current_village;
  }
  ...
  public getVillge_tracts(){
    return this.village_tracts;
  }
  ...
  public genderIcon(){
    return {  1: {name: "md-man", color: "primary"},
              2: {name: "md-woman", color: "secondary"}
            };
  }
  ...
  public do_preload(tbl){
    var tbl_vts = localForage.createInstance({
      ...
      tbl_vts.getItem(tbl).then((result) => {
          //console.log(result);
          this[tbl] = result;
          //console.log(tbl,this[tbl]);
      }, (error) => {
          console.log("ERROR: ", error);
      });
  }
  public pre_loading(){
    var list = ['villages','village_tracts', 'vl_positions'];
    for(var tbl of list){
      this.do_preload(tbl);
    }
  }
  ...
  public loadSetting() {
    this.tbl_setting.getItem('settings')
    .then((result) => {
      if(result){
        this.host =  result['host']; // result ? <Array<Object>> result : [];
    ...
    this.tbl_setting.getItem('village')
    .then((result) => {
      if(result){
        this.current_village =  result ? <Object> result : {};
      }
       //console.log(result);
      }, (error) => {
          console.log("ERROR: ", error);
  ...
  public setSetting(_sett) {
    //console.log(_sett);
    this.tbl_setting.setItem('settings', _sett)
    .then((res) => {
      this.host = (_sett['host'] || this.host);
      this.port = (_sett['port'] || this.port);
      this.lang = (_sett['lang'] || this.lang);
      this.downloaded = (_sett['downloaded'] || this.downloaded);
    }, (error) => {
      console.log("ERROR: ", error);
    });
  }
  ...
  public table(table_name){
    ...
    // this.DataService.table('settings').getItem('settings')
    // .then((res) => {
    //   console.log(res);
    // }, (error) => {
    //   console.log("ERROR: ", error);
  ...
  public z2u(zgtext){
    if(zgtext && zgtext != ''){
      ...
      // For Latest Myanmar3
        ...
        utext =  utext.replace( /(\u102F)(\u102F)/g, "$1"); // clear ု ု = ု 
